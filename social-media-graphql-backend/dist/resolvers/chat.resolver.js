    ...
    Query: {
        ...
        getUsersWithChats: async (_, __, { user }) => {
            ...
            const usersWithChatsAndUnseenMessages = usersWithChats.map((user) => {
                const userUnseenMessages = messages.filter((message) => message.receiverId.toString() === loggedInUserId.toString() &&
            ...
            const sortedUsersWithChats = usersWithChatsAndUnseenMessages.sort((userA, userB) => {
                const userAChatTime = distinctChatUsers.find((item) => item._id.toString() === userA._id.toString()).lastChatTime;
                const userBChatTime = distinctChatUsers.find((item) => item._id.toString() === userB._id.toString()).lastChatTime;
