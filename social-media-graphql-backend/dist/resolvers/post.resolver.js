...
const postResolvers = {
            ...
            try {
                ...
                const notificationPayloads = await Promise.all(notifications.map(async (notification) => {
                    const [creatorUser, targetUser] = await Promise.all([
                        userResolver.Query.findUser(null, {
    ...
    Mutation: {
        ...
        updatePost: async (_, { input }, { user }) => {
            ...
            if (!postId) {
                throwCustomError("Post ID is required to update a post", ErrorTypes.BAD_USER_INPUT);
            }
            if (!message) {
                throwCustomError("new post content is required to update a post", ErrorTypes.BAD_USER_INPUT);
            }
            ...
            if (post.ownerId.toString() !== user._id.toString()) {
                throwCustomError("You don't have permission to update this post", ErrorTypes.UNAUTHENTICATED);
            }
        ...
        likePost: async (_, { input }, { user }) => {
            ...
            try {
                ...
                await createAndPublishNotification(user._id, post.ownerId, "POST", "LIKE", postId);
        ...
        addComment: async (_, { input }, { user }) => {
            ...
            try {
                ...
                // Use the utility function to create and publish the notification
                await createAndPublishNotification(user._id, post.ownerId, "POST", "COMMENT", postId);
                return newComment;
            }
            catch (error) {
    ...
    Subscription: {
        newNotification: {
            subscribe: withFilter(() => pubsub.asyncIterator(NEW_NOTIFICATION), (payload, variables) => {
                const targetUserId = payload.newNotification.targetUser._id.toString();
