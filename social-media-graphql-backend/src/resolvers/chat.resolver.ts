  ...
  Mutation: {
    ...
    markAsSeen: async (_, { input }, { user }) => {
      ...
      if (!messageIds || messageIds.length === 0) {
        throwCustomError(
          "Message IDs are required to mark messages as seen",
          ErrorTypes.BAD_USER_INPUT
        );
      }
      ...
      if (!messages || messages.length !== messageIds.length) {
        throwCustomError(
          "One or more messages not found",
  ...
  Subscription: {
    newMessageChatSubscription: {
      subscribe: withFilter(
        ...
        (payload, variables) => {
          const targetUserId =
            payload.newMessageChatSubscription.receiver._id.toString();
    ...
    seenMessageChatSubscription: {
      subscribe: withFilter(
        ...
        (payload, variables) => {
          const targetUserId =
            payload.seenMessageChatSubscription[0].sender._id.toString();
          const subscriberUserId = variables.receiverId;
          const shouldNotify = targetUserId === subscriberUserId;
          return shouldNotify;
        }
