...
const NewProject = ({mode, projectDataProp }: Props) => {
  ...
  const [name, setName] = useState(projectDataProp?.project_name ? projectDataProp.project_name
     :"");
  const [description, setDescription] = useState(projectDataProp?.description ? projectDataProp.description :"");
  const [due, setDue] = useState(projectDataProp?.due? projectDataProp?.due.toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10));
